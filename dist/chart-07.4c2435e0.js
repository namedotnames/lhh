...
function ready(_ref) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('path').datum(usaDatapoints).attr('d', line).attr('stroke', 'grey').attr('fill', 'none').attr('stroke-width', 2);
    svg.append('text').text('USA').attr('x', 10).attr('y', 35).attr('fill', 'grey').attr('font-size', 12);
    svg.append('path').datum(d.values).attr('d', line).attr('stroke', '#9e4b6c').attr('fill', 'none').attr('stroke-width', 2);
    svg.append('text').text(d.key).attr('x', width / 2).attr('text-anchor', 'middle').attr('fill', '#9e4b6c').attr('y', -15).attr('font-weight', 'bold').attr('font-size', 14);
